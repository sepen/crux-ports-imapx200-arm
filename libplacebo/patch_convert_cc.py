#!/usr/bin/env python3
"""
Overwrite libplacebo's src/convert.cc with a GCC 7-compatible version.

GCC 7.5.0 needs several fixes here that are easier to apply as a full
file replacement than as incremental patches (this took many rounds to
get the brace-matching right):
  - no <charconv> or <version> headers (stubs are provided separately
    via -I on a compat-include directory; this file's #include <charconv>
    still needs to exist syntactically for the SFINAE checks to work)
  - no std::to_chars/from_chars at all, not even for integers (only a
    float/double fallback via ccStrPrintDouble/fast_float existed
    upstream) -- added snprintf/strtoll-based fallbacks for integers
  - __VA_OPT__ is C23 syntax GCC 7 doesn't support -- rewritten with the
    classic GNU ", ##__VA_ARGS__" extension

Usage:
    python3 patch_convert_cc.py /path/to/convert.cc
    python3 patch_convert_cc.py /path/to/libplacebo-6.338.2   (searches for it)
"""
import sys
from pathlib import Path

TARGET_NAME = "convert.cc"
...
static inline int to_chars(char *buf, size_t len, T n, Args ...args)
{
    if constexpr (has_std_to_chars<T>) {
        ...
        // This GCC has no std::to_chars at all, not even for integers
        // (older than the MinGW-w64/GCC<=10 case above anticipated).
        // Plain snprintf covers the integer/hex cases convert.cc actually
        // needs here; args... carries an optional numeric base (16 for
        // hex, otherwise absent/10).
        int base = 10;
        ...
        int ret = std::is_signed_v<T>
            ? snprintf(buf, len, fmt, (long long)n)
            : snprintf(buf, len, fmt, (unsigned long long)n);
        return (ret > 0 && (size_t)ret <= len) ? ret : 0;
    }
}
...
static inline bool from_chars(pl_str str, T &n, Args ...args)
{
    if constexpr (has_std_from_chars<T>) {
...
#if !__has_include(<fast_float/fast_float.h>)
        static_assert(false, "<fast_float/fast_float.h> is required, but not " \\
        ...
        // This GCC has no std::from_chars at all, not even for integers.
        // pl_str.buf is a length-delimited buffer, not a NUL-terminated
        // C string, so copy into a scratch buffer before using strtoll/
        // strtoull (which require NUL termination). args... optionally
        // carries a numeric base (16 for hex, otherwise default 10).
        if (str.len == 0 || str.len >= 64)
            return false;
        char tmp[64];
        ...
        if constexpr (std::is_signed_v<T>) {
            long long v = strtoll(tmp, &end, base);
            if (errno != 0 || end == tmp)
                return false;
            n = (T)v;
        } else {
...
#define CHAR_CONVERT(name, type, ...)                           \\
    ...
    {                                                           \\
        return to_chars(buf, len, n, ##__VA_ARGS__); \\
    }                                                           \\
    bool pl_str_parse_##name(pl_str str, type *n)               \\
    {                                                           \\
        return from_chars(str, *n, ##__VA_ARGS__);   \\
    }
